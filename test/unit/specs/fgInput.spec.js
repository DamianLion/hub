...
describe('formGroupInput.vue', () => {
  it('should work with v-model', done => {
    ...
    // change input value
    input.value = 'world'
    triggerEvent(input, 'input') // eslint-disable-line no-undef
    waitForUpdate(() => { // eslint-disable-line no-undef
      expect(vm.msg).to.equal('world')
    }).end(done)
  })

  it('should accept input attributes', () => {
    const vm = mount(fgInput, {type: 'password', name: 'password', placeholder: 'User password'}) // eslint-disable-line no-undef
    const input = vm.$el.querySelector('input')
